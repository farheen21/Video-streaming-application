...
<section id="content">
  ...
  <section id="table_of_contents">
    ...
    <div id="_table_of_contents">
      <ul>
        ...
        <li><a href="#audit-signatures">Audit Signatures</a></li>
        ...
        <ul>
          ...
          <li><a href="#calculating-meta-vulnerabilities-and-remediations">Calculating Meta-Vulnerabilities and
            Remediations</a></li>
        </ul>
        ...
        <ul>
          ...
          <li><a href="#include-workspace-root"><code>include-workspace-root</code></a></li>
          <li><a href="#install-links"><code>install-links</code></a></li>
        </ul>
  ...
  <div id="_content"><h3 id="synopsis">Synopsis</h3>
    ...
    <p>Note that some vulnerabilities cannot be fixed automatically and will
      require manual intervention or review. Also note that since <code>npm audit fix</code> runs a full-fledged <code>npm
        install</code> under the hood, all configs that
    ...
    <p>To ensure the integrity of packages you download from the public npm registry, or any registry that supports
      signatures, you can verify the registry signatures of downloaded packages using the npm CLI.</p>
    ...
    <p>The npm CLI supports registry signatures and signing keys provided by any registry if the following conventions
      are followed:</p>
    <ol>
      <li>Signatures are provided in the package's <code>packument</code> in each published version within the <code>dist</code>
        object:
      </li>
    </ol>
  ...
  "signatures": [{
    ...
    <p>See this <a href="https://registry.npmjs.org/light-cycle/1.4.3">example</a> of a signed package from the public
      npm registry.</p>
    <p>The <code>sig</code> is generated using the following template: <code>${package.name}@${package.version}:${package.dist.integrity}</code>
      and the <code>keyid</code> has to match one of the public signing keys below.</p>
    <ol start="2">
  ...
  "keys": [{
    ...
    <p>Keys response:</p>
    <ul>
      <li><code>expires</code>: null or a simplified extended ISO 8601 format: <code>YYYY-MM-DDTHH:mm:ss.sssZ</code>
      </li>
    ...
    <p>The expected result will contain a set of advisory objects for each
      ...
      a <code>name</code>, <code>url</code>, <code>id</code>, <code>severity</code>, <code>vulnerable_versions</code>,
    ...
    <p>For example, if the package <code>foo</code> is vulnerable in the range <code>&gt;=1.0.2 &lt;2.0.0</code>, and
      the package <code>bar</code> depends on <code>foo@^1.1.0</code>, then that version
    ...
    <ul>
      <li>Commands that operate on the <code>node_modules</code> tree (install, update, etc.)
        ...
        <em>unless</em> one or more workspaces are specified in the <code>workspace</code> config.
      </li>
    </ul>
    <p>This value is not exported to the environment for child processes.</p>
